...
void TestPitchIO12()
{
  // assumes pitch out 1 is connected to pitch in 2
  // write to pitch out1 -> read pitch in 2 -> write pitch in 2
  Serial.println("Test pitch out 1 -> pitch in 2...");
...
void TestPitchIO21()
{
  // assumes pitch out 1 is connected to pitch in 2
  // write to pitch out1 -> read pitch in 2 -> write pitch in 2
  Serial.println("Test pitch out 2 -> pitch in 1...");
  ...
  for(int octave = 0; octave<5; ++octave)
  {
    ...
    {
      int pitch = 12*octave + notes[idx];
      board.pitchOut.PitchOut(1, 0, pitch);

      board.pitchIn1.Read();
      int pitchin= board.pitchIn1.Get();
      board.pitchOut.PitchOut(0, 0, pitchin);
...
void loop() {
  ...
  // TestButtonIn(board.buttons, 2);
  // TestColorOut(board.led1Out, 4);
  // TestColorOut(board.led2Out, 4);
